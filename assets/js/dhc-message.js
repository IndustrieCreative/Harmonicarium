...
HUM.DHCmsg = class {
     ...
     * @param {hertz}     [hz=false]          - Absolute frequency in Hz; only set for continuum messages (when `continuum` is `true`).
     * @param {midicent}  [mc=false]          - Frequency in midicents; only set for continuum messages.
     * @param {boolean}   [continuum=false]   - `true` if the message was generated by a continuum (fretless) play event
     *                                         and `xtNum` is the sentinel `HUM.DHCmsg.CONTINUUM_XTNUM` (`-1`).
     */
    constructor(source, cmd, type=false, xtNum=false, velocity=false, ctrlNum=false, piper=false, panic=false, tsnap=false, hz=false, mc=false, continuum=false) {
        this.source = source;
        this.cmd = cmd;
        this.type = type;
        this.xtNum = xtNum;
    ...
    /**
     * Creates a `'tone-on'` continuum DHCmsg for a Fundamental Tone.
     *
     * Continuum messages carry absolute `hz`/`mc` values instead of a table-based
     * `xtNum`. The `xtNum` field is set to `HUM.DHCmsg.CONTINUUM_XTNUM` (`-1`).
     *
     * @param {string}   source   - Identifier of the component generating the message.
     ...
     * @param {velocity} velocity - MIDI velocity (1–127).
     *
     * @returns {HUM.DHCmsg} A new continuum `'tone-on'` FT DHCmsg instance.
     */
    static ftONcontinuum(source, hz, mc, velocity) {
        return new HUM.DHCmsg(source, 'tone-on', 'ft',
    ...
    /**
     * Creates a `'tone-off'` continuum DHCmsg for a Fundamental Tone.
     *
     * @param {string}   source            - Identifier of the component generating the message.
     * @param {hertz}    hz                - Absolute frequency in Hz.
     * @param {midicent} mc                - Frequency in midicents.
     * @param {velocity} [velocity=false]  - Release velocity, if applicable.
     *
     * @returns {HUM.DHCmsg} A new continuum `'tone-off'` FT DHCmsg instance.
     */
    static ftOFFcontinuum(source, hz, mc, velocity=false) {
        return new HUM.DHCmsg(source, 'tone-off', 'ft',
            HUM.DHCmsg.CONTINUUM_XTNUM,
            (velocity ? Number(velocity) : false),
            false, false, false, false,
    ...
    /**
     * Creates a `'tone-on'` continuum DHCmsg for a Harmonic/Subharmonic Tone.
     *
     * @param {string}   source   - Identifier of the component generating the message.
     * @param {hertz}    hz       - Absolute frequency in Hz.
     * @param {midicent} mc       - Frequency in midicents.
     * @param {velocity} velocity - MIDI velocity (1–127).
     *
     * @returns {HUM.DHCmsg} A new continuum `'tone-on'` HT DHCmsg instance.
     */
    static htONcontinuum(source, hz, mc, velocity) {
        return new HUM.DHCmsg(source, 'tone-on', 'ht',
            HUM.DHCmsg.CONTINUUM_XTNUM,
            (velocity ? Number(velocity) : false),
            false, false, false, false,
    ...
    /**
     * Creates a `'tone-off'` continuum DHCmsg for a Harmonic/Subharmonic Tone.
     *
     * @param {string}   source            - Identifier of the component generating the message.
     * @param {hertz}    hz                - Absolute frequency in Hz.
     * @param {midicent} mc                - Frequency in midicents.
     * @param {velocity} [velocity=false]  - Release velocity, if applicable.
     *
     * @returns {HUM.DHCmsg} A new continuum `'tone-off'` HT DHCmsg instance.
     */
    static htOFFcontinuum(source, hz, mc, velocity=false) {
        return new HUM.DHCmsg(source, 'tone-off', 'ht',
            HUM.DHCmsg.CONTINUUM_XTNUM,
            (velocity ? Number(velocity) : false),
            false, false, false, false,
...
/**
 * Sentinel `xtNum` value used by all continuum (fretless) tone messages.
 * Continuum tones are stored in the play queue under this key so they can be
 * found and replaced on every pointer-move without colliding with any valid
 * discrete FT/HT index (which are always ≥ −128 but are never −1 in practice).
 *
 * @type {number}
 */
HUM.DHCmsg.CONTINUUM_XTNUM = -1;
